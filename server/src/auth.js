...
export async function ensureHousehold(user) {
  ...
  for (const row of inv.rows) {
    await q(
      'insert into memberships (household_id, user_id, role) values ($1,$2,$3) on conflict do nothing',
      [row.household_id, user.id, row.role]
    );
  }
...
export const requireEdit = (req, res, next) =>
  req.role === 'view' ? res.status(403).json({ error: 'View-only access' }) : next();
