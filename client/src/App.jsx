...
export default function App() {
  ...
  if (session === undefined)
    return (
      <div style={{ padding: 40 }} className="text-muted">
        Loading…
      </div>
    );
  ...
  return (
    <>
      <div className="topbar">
        <div className="topbar-inner">
          <div className="nav-brand" style={{ display: "flex", alignItems: "center", gap: 8, marginRight: "auto" }}>
            <i className="ph ph-wallet" style={{ color: "var(--color-accent)" }} />
            Expensio
          </div>
          <nav className="desktop-only" style={{ display: "flex", gap: "var(--space-4)" }}>
            {NAV.map(([key, label]) => (
              <button key={key} className="navlink" data-active={screen === key} onClick={() => setScreen(key)}>
                {label}
              </button>
            ))}
          </nav>
          ...
          <select
            ...
            onChange={async (e) => {
              await api.saveSettings({ currency: e.target.value });
              await refresh();
            }}
          >
            {CURRENCIES.map((c) => (
              <option key={c.code} value={c.code}>
                {symbolOf(c.code)} {c.code}
              </option>
            ))}
          </select>
          {session.role !== "view" && (
            <button className="btn btn-primary desktop-only" onClick={() => setShowAdd(true)}>
              <i className="ph ph-plus" /> Add expense
            </button>
          )}
        </div>
      </div>
      ...
      {session.role !== "view" && (
        <button className="btn btn-primary fab" onClick={() => setShowAdd(true)} aria-label="Add expense">
      ...
      {showAdd && <AddExpense currency={currency} remaining={summary?.remaining || 0} onClose={() => setShowAdd(false)} onSave={addExpense} />}
    </>
  );
}
